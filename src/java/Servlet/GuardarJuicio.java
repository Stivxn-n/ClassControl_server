...
/**
 * Registra un juicio evaluativo para un aprendiz.
 * Permitido para Instructor, Coordinador y Administrador.
 * Parámetros: idAprendiz (req), valoracion (req), resultadoId,
 * trimestreId, observacion (opcionales).
 */
@WebServlet("/GuardarJuicio")
public class GuardarJuicio extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (!Autorizacion.esAdministrador(request)
            ...
            responder(response, HttpServletResponse.SC_FORBIDDEN,
                    "No tienes permiso para registrar juicios.");
            return;
        }

        Integer idAprendiz = entero(request.getParameter("idAprendiz"));
        String valoracion = texto(request.getParameter("valoracion"));

        if (idAprendiz == null) {
            responder(response, HttpServletResponse.SC_BAD_REQUEST,
                    "Falta el aprendiz del juicio.");
            return;
        }
        if (valoracion == null) {
            responder(response, HttpServletResponse.SC_BAD_REQUEST,
        ...
        boolean ok = new JuiciosDAO().insertar(
                idAprendiz,
        ...
        if (ok) {
            ...
            responder(response, HttpServletResponse.SC_INTERNAL_SERVER_ERROR,
                    "No se pudo guardar el juicio. Intenta de nuevo.");
        }
    }
